...
//     std::shared_ptr<SurakartaAgentMine> my_agent = std::make_shared<SurakartaAgentMine>(game.GetBoard(), game.GetGameInfo(), game.GetRuleManager());
//     std::cout << ANSI_CLEAR_SCREEN << ANSI_MOVE_TO_START;
//     while (!game.IsEnd()) {
//         auto move = agent->CalculateMove();
//         if(game.GetGameInfo()->current_player_==SurakartaPlayer::WHITE)move = my_agent->CalculateMove();
//         game.Move(move);
